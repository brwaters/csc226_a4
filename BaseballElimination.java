...
public class BaseballElimination{
	...
	*/
	public BaseballElimination(Scanner s){
		...
		// Read team details and store them
		
		for (int i = 0; i < numTeams; i++) {
			// For each team, check if it is eliminated
			if (isEliminated(i)) {
				// Add team to eliminated list
			}
		}
	}
	
	private boolean isEliminated(int team) {
		// Build the flow network for the given team
		...
		return isTeamEliminated(team, ff);
	}
	
	private FlowNetwork buildNetwork(int team) {
		// Logic to build the flow network for team
		return null;
	}
	
	private boolean isTeamEliminated(int team, FordFulkerson ff) {
		// Determine if team is eliminated based on the maxflow result
		return false;
	}
